...
.right-column[
Suppose we have a time series $\{y_t\}$ consisting of $T$ observations, i.e., $t=1,\ldots,T$. 

To set up a pseudo-forecasting routine, we divide the sample into two parts, the in-sample set with a total of $R$ observations, such that $R < T$, and the out-of-sample set with the remaining $T-R$ observations. 

If we wanted to evaluate one-step-ahead forecasts, this routine would generate a sequence of forecasts: $\{\hat{y}_{R+1|R},\hat{y}_{R+2|{R+1}},\ldots,\hat{y}_{T|{T-1}}\}$.

Having observed $\{y_{R+1},y_{R+2},\ldots,y_{T}\}$ we can, then, obtain forecast errors, $\hat{e}_{R+j|{R+j-1}} = y_{R+j}-\hat{y}_{R+j|{R+j-1}}$, for all $j = 1,\ldots,T-R$.
]
...
$$\begin{aligned}
\text{MAFE}  = & \frac{1}{P}\sum_{i=1}^{P}|\hat{e}_i|\\
\text{RMSFE} = & \sqrt{\frac{1}{P}\sum_{i=1}^{P}\hat{e}_i^2}
\end{aligned}$$
...
.right-column[
Suppose we neglected the presence of a mean-reverting process, and assumed instead that the random walk best characterizes the time series dynamics. That is, we assumed that our best guess about the next realization of the variable is the most recent observed realization of the variable.
