...
A simplest form of a forecast is *point forecast* (usually the mean of the distribution, because of the assumed loss function).

A more complex but much more informative form of a forecast is *density forecast*.

A relatively less complex form of a forecast (that still gives a bit more detail than point forecast) is *interval forecast*, which is the lower and upper percentiles of the forecast distribution.
]
...
We use 'hat' to indicate that parameter estimates are used to generate the forecast.
