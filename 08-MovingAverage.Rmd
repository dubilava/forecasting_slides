...
.right-column[
The foregoing analysis of a MA(1) process can be generalized to any MA(q) process. Let's focus on MA(2): $$y_t = \mu + \varepsilon_t+\theta_1\varepsilon_{t-1}+\theta_2\varepsilon_{t-2}$$
...
.right-column[
Point forecasts:
- $h=1$: $y_{t+1|t} = E(y_{t+1}|\Omega_t) = \mu + \theta_1 \varepsilon_{t}$
...
- $h=2$: $e_{t+2|t} = y_{t+2}-y_{t+2|t} = \varepsilon_{t+2}+\theta_1 \varepsilon_{t+1}$
- $h>2$: $e_{t+h|t} = y_{t+h}-y_{t+h|t} = \varepsilon_{t+h}+\theta_1 \varepsilon_{t+h-1}$
]
...
- $h>2$: $\sigma_{t+h|t}^2 = E(e_{t+h|t}^2) = \sigma_{\varepsilon}^2(1+\theta_1^2)$
]
...
- $h=1$: $\left[\mu+\theta_1\varepsilon_t-1.96\sigma_{\varepsilon};\mu+\theta_1\varepsilon_t+1.96\sigma_{\varepsilon}\right]$
- $h=2$: $\left[\mu-1.96\sigma_{\varepsilon}\sqrt{1+\theta_1^2};\mu+1.96\sigma_{\varepsilon}\sqrt{1+\theta_1^2}\right]$
- $h>2$: $\left[\mu-1.96\sigma_{\varepsilon}\sqrt{1+\theta_1^2};\mu+1.96\sigma_{\varepsilon}\sqrt{1+\theta_1^2}\right]$
...
- $h \ge q$: $e_{t+h|t} = y_{t+h}-y_{t+h|t} = \varepsilon_{t+h}+\theta_1 \varepsilon_{t+h-1}+\ldots+\theta_q \varepsilon_{t+h-q}$
]
