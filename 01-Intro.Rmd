...
Consider a forecast made in period $t$ for some future period $t+h$, where $h$ denotes the forecast horizon.
...
A simplest form of a forecast is a point forecast (usually a mean of the distribution, but can be a median or, really, any percentile).
...
$$\begin{aligned}
		...
		& + \big[E(y_{t+h}|\Omega_{t}) - g_h(\Omega_{t};\theta)\big]\;~~\text{(model uncertainty)}  \\
		& + \big[g_h(\Omega_{t};\theta)-g_h(\Omega_{t};\hat{\theta})\big]\;~~\text{(parameter uncertainty)}
		\end{aligned}$$
where $g_h(\Omega_{t};\hat{\theta})\equiv \hat{y}_{t+h|t}$
