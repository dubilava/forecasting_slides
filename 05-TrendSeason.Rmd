...
subtitle: "Lecture 5: Forecasting Trends and Seasonality"
...
An exponential trend is equivalent to a linear trend fitted to natural logarithm of the series. For a given time series $\{y_t: t=1,\ldots,T\}$, the natural logarithm is: $z_t - \ln{y_t}$. Some of the benefits of such a transformation are that:
...
Any future realisation of a random variable that i assumed to follow a linear trend model is: $$y_{t+h} = \alpha + \beta (t+h) + \varepsilon_{t+h}.$$ 
An optimal forecast of $y_{t+h}$ is given by: $$y_{t+h|t} = E(y_{t+h}|\Omega_t) = E[\alpha + \beta (t+h) + \varepsilon_{t+h}] = \alpha + \beta (t+h).$$ 
The forecast error is given by: $$e_{t+h|t} = y_{t+h} - y_{t+h|t} = \varepsilon_{t+h}$$ The forecast variance, thus, is given by: $$\sigma_{t+h|t}^2 = E(e_{t+h|t}^2) =  E(\varepsilon_{t+h}^2) = \sigma^2,\;~~\forall\;h$$
...
- they yend to understate uncertainty (at long horizons).
...
Seasonality is a fluctuating, self-repeating pattern within a year that arises from links of technologies, preferences, and institutions to the calendar.

Seasonality is typically modeled as monthly or quarterly pattern, but can also be modeled as a higher frequency pattern (e.g. weekly). Typical examples of time series with apparent seasonal patterns are:
...
Both approches result in an identical fit and forecasts.
...
Any future realisation of a random variable that i assumed to follow a seasonal model is: $$y_{t+h} = \alpha + \sum_{i=1}^{s-1}\delta_i d_{i,t+h} + \varepsilon_{t+h}.$$ 
